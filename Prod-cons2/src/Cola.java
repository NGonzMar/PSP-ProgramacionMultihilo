public class Cola {
    ...
    //  * cuando el consumidor recoga número de cola -> avisará al productor cola está vacía. Consumidor debe esperar a que cola tenga datos
    ...
    public synchronized int get() {
        // Mientras que no esté disponible -> se espera hasta que lo esté (recogemos la excepción)
        while (!disponible) {
            try {
                wait();
        ...
        disponible = false; // flag -> no disponible
        notify(); // Despertar procesos que esperan
        return numero; // Una vez hay valor disponible se devuelve
    }
    ...
    public synchronized void put(int valor) {
        ...
        numero = valor; // Se le asigna al número (dato) el valor que le indicamos por parámetro en 'put()'
        disponible = true; // flag -> disponible
        notify(); // notify(); // Despertar procesos que esperan
    }
}